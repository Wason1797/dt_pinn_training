...
"""Inference and animation export script for time-dependent 3D Room PINN model.

Evaluates trained 4D model (x, y, z, t) -> (u, v, w, p, c) and exports physical
time-lapse series for ParaView visualization.
"""
...
def load_model(
    ...
    if isinstance(checkpoint, dict) and "model_state_dict" in checkpoint:
        ...
        model_config = {"in_features": 4, "out_features": 5, "num_layers": 6, "layer_size": 512}
...
def predict_in_batches(
    ...
    """Run model inference in chunks on (x, y, z, t) inputs."""
    total_pts = coords_t.shape[0]
    preds = np.empty((total_pts, 5), dtype=np.float32)

    with torch.no_grad():
        for start in range(0, total_pts, batch_size):
            end = min(start + batch_size, total_pts)
            batch = torch.from_numpy(coords_t[start:end]).to(device=device, dtype=torch.float32)
            out = model(batch)
...
def export_unsteady_timelapse(
    ...
    """Export 3D grid physical time-lapse series from t = 0 to t = t_max."""
    os.makedirs(output_dir, exist_ok=True)
    xmin, xmax, ymin, ymax, zmin, zmax = bounds

    res = resolution
    print(f"Generating {res}x{res}x{res} grid ({res**3:,} points) across {n_frames} physical time steps...")
    ...
    for frame, t_val in enumerate(time_values):
        ...
        frame_file = f"time_step_{frame:04d}.vtu"
    ...
    pvd_path = os.path.join(output_dir, "timelapse.pvd")
    write_pvd_file(pvd_path, entries)
    return pvd_path


def export_unsteady_particles(
    model: FullyConnected,
    bounds: Tuple[float, float, float, float, float, float],
    t_max: float,
    device: torch.device,
    ...
    """Export time-dependent particle pathlines advected in the unsteady flow field."""
    os.makedirs(output_dir, exist_ok=True)
    xmin, xmax, ymin, ymax, zmin, zmax = bounds
    dt = t_max / n_frames

    # Seed particles at inlet (Windows near ymax)
    ...
    entries: List[Tuple[float, str]] = []
    print(f"  Advecting {n_seeds} particles in unsteady flow for {n_frames} steps (dt={dt:.4f}s)...")

    for frame in range(n_frames):
        current_t = frame * dt
        t_col = np.full((positions.shape[0], 1), fill_value=current_t, dtype=np.float32)
        inp = np.hstack([positions, t_col])
        preds = predict_in_batches(model, inp, device)
        velocity = preds[:, 0:3]
        ...
        # Respawn out-of-bounds particles at inlet
        out_of_bounds = (
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Run inference and physical time-lapse export for time-dependent PINN."
    )
    parser.add_argument(
        "--checkpoint",
        type=str,
        default=default_ckpt,
        help="Path to trained time-dependent model checkpoint (.pth).",
    )
    parser.add_argument(
        "--output-dir",
        type=str,
    ...
    parser.add_argument(
        ...
        type=float,
        metavar=("X", "Y", "Z", "T"),
        help="Probe model predictions at a single spatiotemporal point (x, y, z, t).",
    )
    parser.add_argument(
        "--animate",
    ...
    # Point probe mode
    if args.probe:
        px, py, pz, pt = args.probe
        inp = torch.tensor([[px, py, pz, pt]], dtype=torch.float32, device=device)
        with torch.no_grad():
            pred = model(inp).cpu().numpy()[0]
        v_mag = float(np.linalg.norm(pred[0:3]))
        print(f"\nPrediction at Point ({px:.3f}, {py:.3f}, {pz:.3f}) at t = {pt:.2f}s:")
    ...
    # Single time instance export
    if args.time is not None:
        t_eval = float(args.time)
        res = args.resolution
        print(f"Exporting 3D volume at t = {t_eval:.2f} s ({res}x{res}x{res} grid)...")
        grid_x, grid_y, grid_z = np.mgrid[
        ...
        out_vtu = os.path.join(args.output_dir, f"snapshot_t_{t_eval:.2f}s.vtu")
        vtu.save(out_vtu)
    ...
    if args.animate:
        ...
        print(f"\n{'='*60}")
        print(f"Time-Dependent Animation Export: {args.animate}")
        print(f"Output directory: {anim_dir}")
        print(f"{'='*60}\n")

        if args.animate in ("timelapse", "all"):
            print("[1/2] Exporting 3D physical time-lapse series...")
            pvd = export_unsteady_timelapse(
                model, bounds, t_max, device, os.path.join(anim_dir, "timelapse"),
        ...
        if args.animate in ("particles", "all"):
            ...
            pvd = export_unsteady_particles(
                model, bounds, t_max, device, os.path.join(anim_dir, "particles"),
                n_frames=args.frames
            )
            print(f"  ✓ Unsteady Particles PVD: {pvd}\n")

        print(f"{'='*60}")
        print(f"Animations exported! Open .pvd in ParaView to view physical time evolution.")
        print(f"{'='*60}")
